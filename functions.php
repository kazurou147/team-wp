...
function theme_name_files() {
  ...
  //CSS読み込み slick
  wp_enqueue_style( 'slick-style', get_theme_file_uri('/js/slick.css') ,[] ,wp_get_theme() -> get('Version') );
  //CSS読み込み slick-theme
  wp_enqueue_style( 'slick-theme-style', get_theme_file_uri('/js/slick-theme.css') ,[] ,wp_get_theme() -> get('Version') );

  //jQuery読み込み
  wp_enqueue_script( 'main-jquery', get_theme_file_uri('/js/jquery.js') ,[] ,'ver2.2.4',true);
  //migrate
  wp_enqueue_script( 'migrate-jquery', '//code.jquery.com/jquery-migrate-1.2.1.min.js' ,[] ,'',true);
  //その他（自作）
  wp_enqueue_script( 'original-utility', get_theme_file_uri('/utility.js') ,[] ,'',true);
  //高さを揃える
  ...
  //スクロール時にふわっと表示
  wp_enqueue_script( 'scrollreveal', get_theme_file_uri('/js/scrollreveal.min.js') ,[] ,'',true);
 }
...
//画像のサイズ レスポンシブを考えてwidth 750pxとする
